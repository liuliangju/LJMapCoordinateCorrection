...
@implementation LJMapCoordinateToCorrect
...
+ (CLLocationCoordinate2D)transformFromGCJToBaidu:(CLLocationCoordinate2D)gcjLoc {
    long double z = sqrt(gcjLoc.longitude * gcjLoc.longitude + gcjLoc.latitude * gcjLoc.latitude) + 0.00002 * sin(gcjLoc.latitude * xPi);
    long double theta = atan2(gcjLoc.latitude, gcjLoc.longitude) + 0.000003 * cos(gcjLoc.longitude * xPi);
...
+ (CLLocationCoordinate2D)transformFromGCJToWGS:(CLLocationCoordinate2D)gcjLoc {
    ...
    // The boundary
    double minLat = gcjLoc.latitude - 0.5;
    double maxLat = gcjLoc.latitude + 0.5;
    double minLng = gcjLoc.longitude - 0.5;
    double maxLng = gcjLoc.longitude + 0.5;
